...
# degrees
survival2 = glm(surv ~ scale(size), family=binomial,
...
params_no_26_l$surv_mean = mean(temp_data_no_26_nozeros[temp_data_no_26_nozeros$temp == 20, ]$size)
params_no_26_l$surv_sd = sd(temp_data_no_26_nozeros[temp_data_no_26_nozeros$temp == 20, ]$size)
...
# growth1_4a = lme(sizeNext ~ size + as.factor(temp), random= ~ (size - 1) | individual,
#                     weights=varExp(form=~size | as.factor(temp)), na.action=na.omit, data=temp_data_nozeros_nona)

# growth1_4b = lmer(sizeNext ~ size + as.factor(temp) + (size - 1 | individual),
#                  data=temp_data_nozeros_nona)

# growth1_4c = lmer(sizeNext ~ size + as.factor(temp) + (1 | individual),
#                  data=temp_data_nozeros_nona)
...
growth1_9a = lme(sizeNext ~ temp + size, random= ~(size - 1) | individual,
                    weights=varExp(form=~size),
                    na.action=na.omit, data=temp_data_no_26_nozeros_nona, method="ML")

growth1_9b = lmer(sizeNext ~ temp + size + (size | individual), 
          data=temp_data_no_26_nozeros_nona, REML=F)

growth1_9c = lmer(sizeNext ~ temp + size + (1 | individual), 
          data=temp_data_no_26_nozeros_nona, REML=F)
...
params_no_26_l$growth_ranef_var = getVarCov(growth1_9a)[1] # estimating the variance among slopes for individual frogs
...
### Temporary: Save transmission parameters ###

params_no_26_l$dd_intercept = 0.17
params_no_26_l$dd_slope = 0.18
params_no_26_l$fd_intercept = 0.177 
params_no_26_l$fd_slope = 1.89 
params_no_26_l$z_intercept = 0.025 
params_no_26_l$z_slope = 0.166 
