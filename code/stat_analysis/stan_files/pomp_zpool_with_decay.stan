data{
    ...
    int prob_inf[T, M, N]; // Holds infection probabilities
    real time[T, N]; // Holds time between swabs


    int init_tads; // Initial zoospore load on tadpoles
    int D; // Total observations contributing to the likelihood
    real<lower=0> sigma; // Fix variance of zoospore pool as data. Comment this out and uncomment sigma on line 26 and line 37 for sigma as a parameter.

} parameters{
    
    real<lower=0> b_zpool; // The effect of the zoospore pool on transmission
    real<lower=0> b_density; // The effect of host density on transmission
    real<lower=0> zpool[T, N]; // Holds the true, latent zoospore pool values
    real<lower=0> zpool_init; // Initial zoospore values
    real<lower=0> death_rate; // The survival rate of zoospores
    real<lower=0> mu_z; // Contribution of adults and tadpoled to zpool
    // real<lower=0> sigma; // Variance of unobserved pool
                ...
                for(j in 1:M){ // Loop over individuals
                    ...
                    if(j <= num_frogs[t]){ 
                        ...
                        if(prob_inf[t, j, i - 1] == 0 && prob_inf[t, j, i] != -1){
                            prob_inf[t, j, i] ~ bernoulli(1 - exp(-1*time[t, i]*(b_zpool*log(zpool[t, i - 1] + 1) + b_density*density[t, i - 1]))); 
                        }
                    }
                }
            ...
            if(i != 1){
                for(j in 1:M){ // Loop over individuals
                    ...
                    if(j <= num_frogs[t]){ 
                        ...
                        if(prob_inf[t, j, i - 1] == 0 && prob_inf[t, j, i] != -1){
                            counter <- counter + 1;
                            log_lik[counter] <- bernoulli_log(prob_inf[t, j, i], 1 - exp(-1*time[t, i]*(b_zpool*log(zpool[t, i - 1] + 1) + b_density*density[t, i - 1]))); 
                            dev <- dev - (2* bernoulli_log(prob_inf[t, j, i], 1 - exp(-1*time[t, i]*(b_zpool*log(zpool[t, i - 1] + 1) + b_density*density[t, i - 1]))));
                        }
                    }
                }
